...
/*
 e xtern void iio_device*_unregister(struct iio_dev *indio_dev);
			...
			switch (header->parameter_id) {
				case ELLIPTIC_ULTRASOUND_PARAM_ID_ENGINE_DATA:
					...
					if (((uint8_t *)payload)[23] == (uint8_t)66) {
						pr_info("MIUS receive far signal");
						us_afe_callback(0);
					} else {
						pr_info("MIUS receive near signal");
						us_afe_callback(5);
					}
					break;
				default:
					pr_info("MIUS illegal param id: %u",
							 header->parameter_id);
					break;
			}
		}
...
static int us_proximity_iio_setup(struct device *pdev, struct us_prox_data *data)
{
	...
	*priv_data = data;
	ret = iio_trigger_buffer_setup(idev, iio_pollfunc_us, NULL,
								   &us_buffer_setup_ops);
	if (ret < 0) {
		dev_err(pdev, "iio_trigger_buffer_setup failed: %d\n", ret);
		goto free_iio_p;
	}

	dev_info(pdev, "iio_trigger_buffer_setup success\n");

	ret = iio_device_register(idev);
	if (ret) {
		pr_err("Proximity IIO register fail\n");
		dev_err(pdev, "iio_device_register failed: %d\n", ret);
		goto free_trigger_p;
	}
	dev_info(pdev, "iio_device_register success, device name: %s\n", idev->name);
	dev_info(pdev, "%s: end (success)\n", __func__);
	return ret;
...
static int us_prox_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "us_proximity_iio_setup failed: %d\n", ret);
		return ret;
		}

		{
			int dbg_ret = mius_debug_io_open();
			dev_info(&pdev->dev, "mius_debug_io_open returned: %d\n", dbg_ret);
			if (dbg_ret < 0) {
				dev_err(&pdev->dev, "mius_debug_io_open failed: %d\n", dbg_ret);
				return dbg_ret;
			}
		}
	if (mtk_ipi_register(&scp_ipidev, IPI_IN_ELLIPTIC_ULTRA_0,
		...
		dev_err(&pdev->dev, "scp registration failed\n");
						 }
						 pr_info("%s: end\n", __func__);
